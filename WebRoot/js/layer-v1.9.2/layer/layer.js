...
!function (a, b) {
  ...
  a.layer = {
   ...
   // var innerh=window.innerHeight/2-50;
  ...
  f.pt.vessel = function (a, b) {
    ...
    return f.zIndex = h,
    b([f.shade ? '<div class="layui-layer-shade" id="layui-layer-shade' + d + '" times="' + d + '" style="' + ('z-index:' + (h - 1) + '; background-color:' + (f.shade[1] || '#000') + '; opacity:' + (f.shade[0] || f.shade) + '; filter:alpha(opacity=' + (100 * f.shade[0] || 100 * f.shade) + ');') + '"></div>' : '',
    '<div class="' + g[0] + ' ' + (g.anim[f.shift] || '') + (' layui-layer-' + e.type[f.type]) + (0 != f.type && 2 != f.type || f.shade ? '' : ' layui-layer-border') + ' ' + (f.skin || '') + '" id="' + g[0] + d + '" type="' + e.type[f.type] + '" times="' + d + '" showtime="' + f.time + '" conType="' + (a ? 'object' : 'string') + '" style="z-index: ' + h + '; width:' + f.area[0] + ';height:' + f.area[1] + (f.fix ? '' : ';position:absolute;') + '">' + (a && 2 != f.type ? '' : k) + '<div class="layui-layer-content' + (0 == f.type && - 1 !== f.icon ? ' layui-layer-padding' : '') + (3 == f.type ? ' layui-layer-loading' + f.icon : '') + '">' + (0 == f.type && - 1 !== f.icon ? '<i class="layui-layer-ico layui-layer-ico' + f.icon + '"></i>' : '') + (1 == f.type && a ? '' : f.content || '') + '</div><span class="layui-layer-setwin">' + function () {
  ...
  f.pt.creat = function () {
    ...
    switch ('string' == typeof b.area && (b.area = 'auto' === b.area ? [
      ...
      case 0:
        ...
        break;
      case 2:
        var h = b.content = i ? b.content : [
          b.content || 'http://sentsin.com?from=layer',
          'auto'
        ];
  ...
  e.run = function () {
    ...
    layer.open = function (a) {
    	//修改offset 弹出页面是固定在顶部偏下150px
    	//其他为屏幕居中 
    	var type=a.type||0;
    	if	(type==2){
    		a.offset='50px';
    	}else{
    		var innerh=window.innerHeight/2-150;
    	 	a.offset=innerh+'px';	
    	}
