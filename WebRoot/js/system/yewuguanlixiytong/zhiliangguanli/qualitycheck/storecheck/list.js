...
$(function() {
	...
	grid = $('#magazineGrid').datagrid({
		url : rootPath+'/qualityManager/findStoreByPage.shtml',
		method : 'POST',
...
function addUI(){
	pageii = layer.open({
		...
		area : [ "1200px", "600px"],
		offset:"40px",
		content : rootPath + '/qualityManager/addStoreCheckUI.shtml',
		end :function(){
...
function editUI(){
	...
	pageii = layer.open({
		...
		area : [ "1200px", "600px" ],
		offset:"40px",
		content : rootPath + '/qualityManager/editStoreCheckUI.shtml?id='+selectRow.id,
		end :function(){
			grid.datagrid("reload");
		}
	});
...
function exportFun(){
	var selectRow =grid.datagrid('getSelected');
	if(selectRow){
		window.open(rootPath+"/qualityManager/show.shtml?pId="+selectRow.id);
	}
	else{
		layer.msg("请选择一行");
	}
