...
$(function() {
	...
	grid = $('#magazineGrid').datagrid({
		url : 'shougouxinxi/findByPage.shtml',
		method : 'POST',
	...
	$("#add").click("click", function() {
		pageii = layer.open({
			...
			maxmin : true,
			area : [ "1200px", "600px" ],
			content : rootPath + "/shougouxinxi/messageAddPage.shtml",
			end : function() {
	...
	$("#edit").click("click", function() {
		...
		if (selectRow) {
			pageii = layer.open({
				...
				maxmin : true,
				area : [ "1200px", "600px" ],
				content : rootPath + '/shougouxinxi/messageEditPage.shtml?id='+ selectRow.id,
				end : function() {
					grid.datagrid("reload");
				}
			});
	...
	$("#remove").click("click", function() {
		...
		if (selectRow) {
			layer.confirm('确认删除？', function(index) {
				...
				var url = rootPath + '/shougouxinxi/deleteBuyMsg.shtml';
