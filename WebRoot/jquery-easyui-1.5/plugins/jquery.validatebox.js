...
(function($) {
	...
	$.fn.validatebox.defaults = {
		...
		rules : {
			...
			// 手机号码验证
			mobile: { //验证手机号    
                validator: function(_t01){  
                 return /^1[3-8]+\d{9}$/.test(_t01);  
                },     
                message: 'Please enter a valid mobile.'    
            }, 
           // 电话号码验证
           telephone : {
            validator : function(_t02) {
             // 电话号码格式010-12345678
            	return /^\d{3,4}\-?\d{7,8}$/.test(_t02);
            },
            message : "Please enter a valid telephone."
           },
          // 验证QQ,从10000开始
           qq : {
        	 validator : function(_t03) {
        	    return /^[1-9]\d{4,10}$/i.test(_t03);
        	  },
        	 message : 'Please enter a valid QQ.'
           },
          // 验证年龄
           age : {
        	 validator : function(_t04) {
        	  return /^(?:[1-9][0-9]?|1[01][0-9]|120)$/i
        	    .test(_t04);
        	 },
        	 message : 'Age must Integer and between 0 and 120'
           },
           // 验证身份证
           IDCard : {
        	   validator : function(_t05) {
        		   //var len = $.trim(_t05).length;
        		   return /^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$|^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/.test(_t05);
