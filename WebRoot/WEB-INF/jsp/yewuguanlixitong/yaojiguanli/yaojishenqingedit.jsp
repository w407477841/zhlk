...
<body style="background-color:#d9edf7">
		...
		<form id="form" name="form" method="post" action="">
				...
				<div class="col-md-10">
					<div class="has-warning">
						<input type="text" class="form-control easyui-validatebox"
							missingMessage="请输入申请数量" data-options="required:true" id="number"
							onkeyup="this.value=this.value.replace(/\D/g,'')"
							onblur="validIOK(this)"
							value="${yaoJiShenQingFormMap.maQuantity}"
							name="yaoJiShenQingFormMap.maQuantity" placeholder="请输入 ：领用数量"
							maxlength="9">
					</div>
				</div>
			</div>
			<div class="form-group col-md-6">
				...
				<div class="col-md-10">
					<div class="has-warning">
						<input type="text" class="form-control easyui-validatebox"
							missingMessage="请输入申请人" data-options="required:true" id="auditor"
							name="yaoJiShenQingFormMap.maApplicant"
							value="${yaoJiShenQingFormMap.maApplicant}"
							placeholder="请输入 ：申请人" maxlength="64">
					</div>
				</div>
			</div>
			<div class="form-group col-md-12">
				...
				<div class="col-md-11">
					<div class="has-warning">
						<textarea rows="4" cols="20" class="form-control"
							...
							maxlength="256">${yaoJiShenQingFormMap.maUsed }</textarea>
					</div>
				</div>
			</div>
			<div class="form-group col-md-12">
				...
				<div class="col-md-11">
					<div class="has-warning">
						<textarea rows="4" cols="20" class="form-control"
							...
							maxlength="256">${yaoJiShenQingFormMap.remark }</textarea>
					</div>
				</div>
			</div>

			<c:if test="${yaoJiShenQingFormMap.maProcess == '审批未通过' }">
				<div class="form-group col-md-12">
					<div class="col-md-1 div_label" style="color:red;">审批意见</div>
					<div class="col-md-11">
						<div class="has-danger div_label"
							style="color:red; text-align: left;">
							${yaoJiShenQingFormMap.maOpinion }</div>
					</div>
				</div>
			</c:if>
	...
	<script type="text/javascript">
		...
		var sel = new myselect({
			...
			option_html : [ "wName" ],
			option_html_prefix : ",",
			selected_val : ${yaoJiShenQingFormMap.maWLNo}
		}, function() {
			//给change事件添加方法
			$("#selectName").html("");
		...
		function findDrug(location) {
			new myselect({
				...
				option_html : [ "mrName" ],
				option_html_prefix : ",",
				selected_val : ${yaoJiShenQingFormMap.maMrId}
			}, function() {
				//给change事件添加方法
		...
		function _submit() {
			$("#form")
					.attr("action",
							"${ctx}/yaojilingyongshenqing/edit.shtml?yaoJiShenQingFormMap.maProcess=1");
		...
		function _approve() {
			$("#form")
					.attr("action",
							"${ctx}/yaojilingyongshenqing/edit.shtml?yaoJiShenQingFormMap.maProcess=2");
			if ($("#form").form("validate")) {
				$("#form").submit();
			}
		}
