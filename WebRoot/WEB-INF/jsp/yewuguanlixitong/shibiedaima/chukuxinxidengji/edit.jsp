...
<body style="background-color:#d9edf7">
	<div class=" col-md-12">
		<form id="form" name="form" method="post" action="${ctx}/chukuxinxidengji/edit.shtml">
			<input type="hidden" name="token" value="${token}">
			<input type="hidden" name="OutRegisterFormMap.id" value="${OutRegisterFormMap.code}">
			
			<input type="hidden" class="temp" name="" value="${OutRegisterFormMap.location}">
			<input type="hidden" class="temp" name="" value="${OutRegisterFormMap.houseid}">
			...
			<h3 class="animated rubberBand">基础信息</h3>
					...
					<div class="col-md-9">
						<div class="has-warning">
							<input id="" type="text" class=" form-control " 
								name="OutRegisterFormMap.code" value="${OutRegisterFormMap.code}" placeholder="货位代码">
						</div>
					</div>
				</div>	
				...
				<div class="form-group col-md-6">
					...
					<div class="col-md-9">
						<div class="has-warning">
							<input id="" type="text" class=" form-control " 
								name="OutRegisterFormMap.roomName" value="${OutRegisterFormMap.roomName}" placeholder="廒间名称">
						</div>
					</div>
				</div>
				<div class="form-group col-md-6">
					...
					<div class="col-md-9">
						<div class="has-warning">
							<input id="" type="text" class=" form-control " 
								name="OutRegisterFormMap.positionName" value="${OutRegisterFormMap.positionName}" placeholder="货位名称">
						</div>
					</div>
				</div>
					...
					<div class="col-md-9">
						<div class="has-warning">
							<input id="start" type="text" class=" inline laydate-icon  form-control " 
								name="OutRegisterFormMap.outDate" value="${OutRegisterFormMap.outDate}" placeholder="出库时间">
						</div>
					</div>
				</div>	
					...
					<div class="col-md-9">
						<div class="has-warning">
							<input id="amounts" type="text" class=" form-control " 
								name="OutRegisterFormMap.amounts" value="${OutRegisterFormMap.amounts}" placeholder="出库数量">
						</div>
					</div>
				</div>
...
$.ajax({
	...
	$("#selectLocation").on("change",function(event){
		var value=this.value//||"${BusinessAgreementFormMap.location}";
			if(!value){
			value="${OutRegisterFormMap.location}";
			var flag=true;
			}
				...
				rs=JSON.parse(rs);
					if(rs.success){
						for ( var i = 0; i < rs.result.length; i++) {
							...
							if(rs.result[i].id=="${fn:split(OutRegisterFormMap.houseid,',')[0]}"){
							ops.selected=true;
							}
							sel.add(ops);
						}
