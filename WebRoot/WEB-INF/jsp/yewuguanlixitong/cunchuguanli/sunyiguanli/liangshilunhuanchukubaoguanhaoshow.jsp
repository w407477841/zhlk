...
<html>
 ...
 $("#content").html(loadingImg());
  $.ajax({
		url : rootPath+"/liangshilunhuanchukubaoguanhao/export.shtml",
		type : 'post',
		dataType : 'html',
		data:{pId:'${pId}',showType:'html'},
		error : function(data) {
			if (data.status = 404)
  ...
  $("#pdf").click("click",function(){
	  var pId = '${pId}';
	  window.open(rootPath+"/liangshilunhuanchukubaoguanhao/export.shtml?pId="+pId+"&showType=pdf");
  });
  $("#excel").click("click",function(){
	  var pId = '${pId}';
	  window.open(rootPath+"/liangshilunhuanchukubaoguanhao/export.shtml?pId="+pId+"&showType=excel");
  });
  $("#word").click("click",function(){
	  var pId = '${pId}';
	  window.open(rootPath+"/liangshilunhuanchukubaoguanhao/export.shtml?pId="+pId+"&showType=word");
  });
