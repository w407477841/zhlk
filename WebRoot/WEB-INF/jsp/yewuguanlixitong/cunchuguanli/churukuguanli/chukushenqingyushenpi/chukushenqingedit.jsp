...
<body style="background-color:#d9edf7">
	...
	<div class=" col-md-12">
		...
		<form id="form" name="form" method="post"
			action="${ctx}/chukushenqing/edit.shtml">
					...
					<div class="col-md-9">
						<div class="has-warning">
							...
							missingMessage="请输入申请单编号" data-options="required:true"
								name="chuKuShenQingFormMap.gsaApplyBill"
								value="${chuKuShenQingFormMap.gsaApplyBill }"
					...
					<div class="col-md-9">
						<div class="has-warning">
							...
							missingMessage="请输入买受人" data-options="required:true"
								...
								value="${chuKuShenQingFormMap.gsaPurchaser }"
					...
					<div class="col-md-9">
						<div class="has-warning">
							<select class="form-control"
								...
								<c:if
									test="${chuKuShenQingFormMap.gsaPaymentSituation == '未到账' }">
									<option value="未到账" selected="selected">未到账</option>
									<option value="已到账">已到账</option>
								</c:if>
								<c:if
									test="${chuKuShenQingFormMap.gsaPaymentSituation == '已到账' }">
									<option value="未到账">未到账</option>
									<option value="已到账" selected="selected">已到账</option>
								</c:if>
							</select>
						</div>
					</div>
	...
	<script type="text/javascript">
		...
		var sel = new myselect({
			...
			option_html : [ "wName" ],
			option_html_prefix : ",",
			selected_val : "${chuKuShenQingFormMap.wLocation}"
		}, function() {
			//给change事件添加方法
			$("#warehouseName").html("");
		...
		function findWarehouse(location) {
			new myselect({
				...
				option_html : [ "wName" ],
				option_html_prefix : ",",
				selected_val : "${chuKuShenQingFormMap.gsaWId}"
			}, function() {
				//给change事件添加方法
				getWeight();
			}, function() {
		...
		var sele = new myselect({
			...
			option_html : [ "text" ],
			option_html_prefix : ",",
			selected_val : "${chuKuShenQingFormMap.gsaApplyDepartment}"
		}, function() {
			//给change事件添加方法
		}, function() {
			//初始化执行方法
		...
		var sel = new myselect({
			...
			option_html_prefix : ",",
			selected_val : "${chuKuShenQingFormMap.gsaAgreementBill}"
		}, function() {
			//给change事件添加方法
		}, function() {
			//初始化执行方法
