...
<body class="myfont">
	<div class="body_all">
		<div class="box col-sm-12 no-padding">
			<div class="box-inner">
				<div class="box-header well">
					<h2>
						<i class="glyphicon glyphicon-tint"></i> <span>三维展示</span>
					</h2>
				</div>
				<div class="box-content no-padding" style="padding: 0 0 0 0;">
					<embed width='100%' height='380' name='Scene1' id="3dshow"
		...
		<div class="box col-sm-12 no-padding">
			<div class="box-inner">
               ...
               <div class="row">
						...
						class="fa fa-caret-down fa-rotate-90 fa-3x"
						onclick="ChangeViewer(0,-5)"> </a>
						</div>
						<div style="width: 14%; float: left;">
						 <a href="javascript:void(0)"
						class="fa fa-caret-down fa-rotate-270 fa-3x"
						onclick="ChangeViewer(0,5)"> </a> 
						</div>
						<div style="width: 14%; float: left;">
						<a href="javascript:void(0)"
						class="fa fa-caret-down fa-flip-vertical fa-3x"
						onclick="ChangeViewer(10,0)"> </a> 
						</div>
						<div style="width: 14%; float: left;">
						<a href="javascript:void(0)"
						class="fa fa-caret-down fa-3x" onclick="ChangeViewer(-10,0)"></a>
						</div>
						<div style="width: 16%; float: left;">
						<a href="javascript:void(0)" id="contr" class="fa fa-play fa-2x"
						style="padding-left: 10px; padding-right: 10px;"
						onclick="clickxuanzhuan()"> </a> 
						</div>
						<div style="width: 14%; float: left;">
						<a href="javascript:void(0)"
						style="padding-right: 10px;" class="fa fa-search-plus fa-2x"
						onclick="ZoomPicture(1);"> </a> 
						</div>
						<div style="width: 14%; float: left;">
						<a href="javascript:void(0)"
						class="fa fa-search-minus fa-2x" onclick="ZoomPicture(-1);"> </a>
						</div>
						</div>
						</div>
				</div>
		...
		<div class="box col-sm-12 no-padding">
			<div class="box-inner">
				...
				<div class="box-content">
					<div class="row">
						<div class="col-sm-12">
							<div id="cengshuTd" style="width: 25%; float: left;">
								<div style="font-weight: bold;">显示控制</div>
							</div>
							<div id="pingjunwen"
								style="width: 25%; float: left;color:#6E8B3D;">
								<div style="font-weight: bold;">平均温</div>
							</div>
							<div id="zuigaowen"
								style="width: 25%; float: left;color:#CD661D;">
								<div style="font-weight: bold;">最高温</div>
							</div>
							<div id="zuidiwen"
								style="width: 25%; float: left;color: #708090;">
			...
			<div class="box-inner">
				...
				<div class="box-content">
					<div class="row div-control-height">
						<div style="width: 25%; float: left;">采集时间:</div>
						<div style="width: 75%; float: left;">${warehouseFormMap.cjsj}</div>
					</div>
					<div class="row div-control-height">
						<div style="width: 25%; float: left;">仓内温度:</div>
						<div style="width: 25%; float: left;" id="cnwd"></div>
						<div style="width: 25%; float: left;">仓外温度:</div>
						<div style="width: 25%; float: left;" id="cwwd"></div>
					</div>
					<div class="row div-control-height">
						<div style="width: 25%; float: left;">仓内湿度:</div>
						<div style="width: 25%; float: left;" id="cnsd"></div>
						<div style="width: 25%; float: left;">仓外湿度:</div>
						<div style="width: 25%; float: left;" id="cwsd"></div>
					</div>
					<div class="row div-control-height">
						<div style="width: 25%; float: left;">最高温度:</div>
						<div style="width: 25%; float: left;" id="zgwd"></div>
						<div style="width: 25%; float: left;">最低温度:</div>
						<div style="width: 25%; float: left;" id="zdwd"></div>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript">
	...
	function Init() {
		...
		if (useSVG) {
			...
			S = new Scene3D(SVGObjects[0], 0, 380, 380);
		} else {
		...
		for (h = 0; h < showtemp.length; h++) {
				...
				for (m = 0; m < ymax; m++) {
					for (n = 0; n < xmax; n++) {
						...
						if (Number(arraylist[(zmax - h - 2) * xmax * ymax + n
								+ xmax * m]) < 300
								&& Number(arraylist[(zmax - h - 2) * xmax
										* ymax + n + xmax * m]) > max)
			...
			if (num > 0 && !isNaN(sum)) {
				if ((sum / num).toFixed(2) >= 300) {
					$("#pingjunwen").append(
							"<div class='div-control-height'>D-E</div>");
				} else {
					$("#pingjunwen").append(
				...
				$("#pingjunwen")
						.append(
								"<div class='div-control-height' style='color:black;'>D-E</div>");

			if (max > 50) {
				if (max >= 300) {
					$("#zuigaowen")
							.append(
									"<div class='div-control-height' style='color: red;'>D-E</div>");
				} else {
					$("#zuigaowen").append(
	...
	function clickxuanzhuan() {
		if (isRotating) {
			isRotating = false;
			$("#contr").attr('class', 'fa fa-play fa-2x');
			clearInterval(a);
		} else {
			isRotating = true;
			$("#contr").attr('class', 'fa fa-stop fa-2x');
			Rotate();
...
<script type="text/javascript">
	...
	function drowpanel(colorarr, namearr) {
		...
		for (var i = 0; i < colorarr.length; i++) {
			results += "<div style='width:30%; height:20px;display:inline; margin-top:10px; float:left; text-align:right;'>"
					+ namearr[i] + "</div>";
			results += "<div style='width:20%; height:20px;background-color:"
					+ colorarr[i]
					+ ";display:inline;float:left; margin-top:10px;'></div>";
		}
		results += "</div>";
		$("#contentss").html('');
	...
	$(function() {
		...
		var cnwdStr = '${warehouseFormMap.wTemperature}';
		var cwwdStr = '${warehouseFormMap.gTemperature}';
		var zgwdStr = '${warehouseFormMap.maxTemperature}';
		var zdwdStr = '${warehouseFormMap.minTemperature}';
		
		var cnwd = cnwdStr == ''?800:eval(cnwdStr).toFixed(2);
		var cwwd = cwwdStr == ''?800:eval(cwwdStr).toFixed(2);
		var zgwd = zgwdStr == ''?800:eval(zgwdStr).toFixed(2);
		var zdwd = zdwdStr == ''?800:eval(zdwdStr).toFixed(2);
