...
<body style="background-color:#d9edf7">
	<div class=" col-md-12">
		<div class=" col-md-6">
			<h3 class="animated rubberBand">仓库点位修改</h3>
		</div>
	</div>
	<div class=" col-md-12">
		<form id="form" name="form" method="post"
			action="${ctx}/cangkudianwei/edit.shtml">
			<input type="hidden" id="id" name="cangKuDianWeiFormMap.id"
				value="${cangKuDianWeiFormMap.id}"> <input type="hidden"
					...
					<div class="col-md-10">
						<div class="has-warning">
							<input type="text" class="form-control easyui-validatebox"
								...
								name="cangKuDianWeiFormMap.name"
								value="${cangKuDianWeiFormMap.name }" placeholder="请输入 ：点位名称">
						</div>
					</div>
				</div>
				<div class="form-group col-md-12">
					...
					<div class="col-md-10">
						<div class="has-warning">
							<select id="selectLx" data-rel="chosen" data-placeholder="点位类型"
								...
								<option value=""></option>
								<option value="1"
									<c:if test="${cangKuDianWeiFormMap.lx=='1'}">selected="selected"</c:if>>
									仓库点位</option>
								<option value="2"
									<c:if test="${cangKuDianWeiFormMap.lx=='2'}">selected="selected"</c:if>>仓外传感器点位</option>
							</select>
						</div>
					</div>
				</div>
					...
					<div class="col-md-10">
						<div class="has-warning">
							<input type="text" class="form-control easyui-validatebox"
								...
								name="cangKuDianWeiFormMap.cLeft"
								value="${cangKuDianWeiFormMap.cLeft }" placeholder="请输入 ：横轴位置">
						</div>
					</div>
				</div>
				<div class="form-group col-md-12">
					...
					<div class="col-md-10">
						<div class="has-warning">
							<input type="text" class="form-control easyui-validatebox"
								...
								name="cangKuDianWeiFormMap.cTop"
								value="${cangKuDianWeiFormMap.cTop }" placeholder="请输入 ：纵轴位置">
						</div>
					</div>
				</div>
			</div>
			<div class="form-group col-md-6">
				<div id="img_cangku">
					<div id="shexiangtouImg"
						style='width: 20px; height: 20px; position: absolute; left: ${cangKuDianWeiFormMap.cLeft}; top: ${cangKuDianWeiFormMap.cTop}; background: transparent url("/zhlk/img/cangku/jk.png") repeat scroll 0% 0% / 100% 100%; color: white; cursor: pointer;'></div>
				</div>
			</div>
	...
	<script type="text/javascript">
		(function() {
			var sel = new myselect({
				...
				option_html : [ "wName" ],
				option_html_prefix : ",",
				selected_val : '${cangKuDianWeiFormMap.location_id}'
				...
				$.ajax({
					...
					success : function(data) {
						$("#img_cangku").css("background",
								"url(${ctx}/" + data + ")").css(
								"background-size", "100% 100%").css("height",
								"300px");
					}
			...
			function selChangeInit() {
				if (validate()) {
					new myselect({
						...
						option_html : [ "wName" ],
						option_html_prefix : ",",
						selected_val : '${cangKuDianWeiFormMap.warehouse_id}'
					...
					new myselect({
						...
						option_html : [ "name" ],
						option_html_prefix : ",",
						selected_val : '${cangKuDianWeiFormMap.shipin1}'
					}, function() {
					...
					new myselect({
						...
						option_html : [ "name" ],
						option_html_prefix : ",",
						selected_val : '${cangKuDianWeiFormMap.shipin2}'
			...
			$("#img_cangku").css(
					"background",
					"url(${ctx}/img/cangku/" + $("#selectLocation").val()
							+ ".jpg)").css("background-size", "100% 100%").css(
					"height", "300px");
			$("#selectLocation").change(
					function() {
						$("#img_cangku").css(
								"background",
								"url(${ctx}/img/cangku/" + $(this).val()
										+ ".jpg)").css("background-size",
